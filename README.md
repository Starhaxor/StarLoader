...
The dashboard displays only the current safe profile fields: email, account status, product, license status and expiry, maximum devices, device status, the shortened server device ID, session expiry, and the local masked display HWID. It never receives or displays the bearer token, password, license key, HMAC values, TPM keys/signatures, or raw hardware serials; the client keeps the token only in its in-memory authentication state.
