...
DeviceProofBuilder builder(FakeProofSigner &signer, bool localDevelopment = false)
{
...
private slots:
    ...
    void permitsOnlyExplicitNumericLoopbackHttp();
...
void DeviceProofTest::buildsExactRequestBoundProof()
{
    ...
    const ProofResult result = builder(signer).build(
        ...
        QStringLiteral("access.token.value"),
        expectedThumbprint);
    ...
    QCOMPARE(payload.value(QStringLiteral("ath")).toString(), QStringLiteral("m7YjGxnm6c-mlbFRw5DcZNW6xR4W5Gsdb6H3waxOKhY"));
...
void DeviceProofTest::rejectsMalformedPublicBlob()
{
    ...
    verifyRejected(builder(signer).build(QStringLiteral("GET"), QUrl(QStringLiteral("https://api.example.test/v1/me")),
                                         QStringLiteral("access.token.value"), QStringLiteral("0r62zgBj277RicA3LnaBKQ5_9RCDIomrlbpWjO0QTG0")));
...
{
    FakeProofSigner signer;
    verifyRejected(builder(signer).build(QStringLiteral("GET"), QUrl(QStringLiteral("http://127.0.0.1:8080/v1/me")),
                                         QStringLiteral("access.token.value"), QStringLiteral("0r62zgBj277RicA3LnaBKQ5_9RCDIomrlbpWjO0QTG0")));
    verifyRejected(builder(signer).build(QStringLiteral("GET"), QUrl(QStringLiteral("https://user@example.test/v1/me")),
                                         QStringLiteral("access.token.value"), QStringLiteral("0r62zgBj277RicA3LnaBKQ5_9RCDIomrlbpWjO0QTG0")));
}

void DeviceProofTest::permitsOnlyExplicitNumericLoopbackHttp()
{
    FakeProofSigner signer;
    QVERIFY(builder(signer, true).build(QStringLiteral("GET"), QUrl(QStringLiteral("http://127.0.0.1:8080/v1/me")),
                                       QStringLiteral("access.token.value"), QStringLiteral("0r62zgBj277RicA3LnaBKQ5_9RCDIomrlbpWjO0QTG0")).valid);
    verifyRejected(builder(signer, true).build(QStringLiteral("GET"), QUrl(QStringLiteral("http://localhost:8080/v1/me")),
                                               QStringLiteral("access.token.value"), QStringLiteral("0r62zgBj277RicA3LnaBKQ5_9RCDIomrlbpWjO0QTG0")));
    verifyRejected(builder(signer, true).build(QStringLiteral("GET"), QUrl(QStringLiteral("http://192.0.2.1:8080/v1/me")),
                                               QStringLiteral("access.token.value"), QStringLiteral("0r62zgBj277RicA3LnaBKQ5_9RCDIomrlbpWjO0QTG0")));
}
...
void DeviceProofTest::rejectsEmptyTokenAndThumbprintMismatch()
{
    ...
    verifyRejected(builder(signer).build(QStringLiteral("GET"), QUrl(QStringLiteral("https://api.example.test/v1/me")),
                                         QStringLiteral("access.token.value"), QStringLiteral("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA")));
    QCOMPARE(signer.signCalls, 0);
}
